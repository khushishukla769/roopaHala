...
import { imageUrl, API_KEY } from "../../Constants/Constance";
...
function RowPost(props) {
  ...
  return (
    ...
    >
      ...
      {movies[0] ? (
        <>
          <h1 className={`text-common pb-4 xl:pb-0 text-base sm:text-2xl md:text-4xl`}>
            {props.title}
          </h1>
          ...
          <Swiper
            ...
            spaceBetween={isTop10Section ? 60 : 8}
            slidesPerView={isTop10Section ? 5 : 6.1}
            navigation
          ...
          >
            {movies.map((obj, index) => {
              ...
              return (
                <SwiperSlide
                  key={index}
                  className={isTop10Section ? "ml-48 large relative" : "large"}
                  onClick={() => handleMoviePopup(obj)}
                >
                  {isTop10Section && (
                    <span
                      className="absolute -left-36 top-1/2 -translate-y-1/2 z-0
                    text-[20rem] font-bold font-museo
